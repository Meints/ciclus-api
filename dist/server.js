...
var env = createEnv({
  server: {
    ...
    JWT_EXPIRES_IN: z.coerce.number().default(60 * 60 * 24 * 7),
    // 7 dias
...
var loginSchema = z3.object({
  ...
  password: z3.string().min(8, PASSWORD_ERROR).regex(PASSWORD_REGEX, PASSWORD_ERROR)
}).strict();
...
var createServiceSchema = z25.object({
  contractId: z25.string().uuid().optional(),
  customerId: z25.string().uuid("Cliente \xE9 obrigat\xF3rio"),
  serviceType: z25.string().min(1, "Tipo de servi\xE7o \xE9 obrigat\xF3rio"),
  scheduledDate: z25.string().min(1, "Data agendada \xE9 obrigat\xF3ria"),
  employeeId: z25.string().uuid().optional(),
  estimatedDurationMinutes: z25.number().int().min(1).optional(),
  equipmentIds: z25.array(z25.string().uuid()).optional().default([])
}).strict();
...
var equipmentNoteSchema = z26.object({
  equipmentId: z26.string().uuid(),
  note: z26.string().max(500)
});
var completeServiceSchema = z26.object({
  executionNotes: z26.string().min(1, "Observa\xE7\xF5es s\xE3o obrigat\xF3rias").max(2e3).optional(),
  durationMinutes: z26.number().int().positive().optional(),
  equipmentNotes: z26.array(equipmentNoteSchema).optional()
}).strict();

// src/modules/services/dtos/cancel-service.dto.ts
import { z as z27 } from "zod";
var cancelServiceSchema = z27.object({
  reason: z27.string().min(1, "Motivo do cancelamento \xE9 obrigat\xF3rio").max(500)
}).strict();

// src/modules/services/dtos/reschedule-service.dto.ts
import { z as z28 } from "zod";
var rescheduleServiceSchema = z28.object({
  scheduledAt: z28.string().min(1, "Nova data \xE9 obrigat\xF3ria"),
  estimatedDurationMinutes: z28.number().int().min(1).optional()
}).strict();

// src/modules/services/dtos/service-filters.dto.ts
import { z as z29 } from "zod";
var serviceFiltersSchema = z29.object({
  page: z29.coerce.number().int().positive().optional(),
  pageSize: z29.coerce.number().int().min(1).max(100).optional(),
  status: z29.enum(["SCHEDULED", "IN_PROGRESS", "COMPLETED", "CONFIRMED", "CANCELLED"]).optional(),
  employeeId: z29.string().uuid().optional(),
  customerId: z29.string().uuid().optional(),
  contractId: z29.string().uuid().optional(),
  dateStart: z29.string().optional(),
  dateEnd: z29.string().optional()
}).strict();

// src/modules/services/dtos/link-equipment.dto.ts
import { z as z30 } from "zod";
var linkEquipmentSchema = z30.object({
  equipmentIds: z30.array(z30.string().uuid()).min(1, "Selecione pelo menos um equipamento")
}).strict();

// src/modules/services/dtos/update-service.dto.ts
import { z as z31 } from "zod";
var updateServiceSchema = z31.object({
  contractId: z31.string().min(1).optional(),
  customerId: z31.string().min(1).optional(),
  serviceType: z31.string().min(1).optional(),
  scheduledDate: z31.string().min(1).optional(),
  employeeId: z31.string().nullable().optional(),
  equipmentIds: z31.array(z31.string()).optional()
}).strict();
...
var confirmParamsSchema = z32.object({
  token: z32.string()
});
var confirmBodySchema = z32.object({
  name: z32.string().max(255).optional().default(""),
  document: z32.string().regex(/^\d+$/, "Documento deve conter apenas n\xFAmeros").min(11, "Documento deve ter no m\xEDnimo 11 d\xEDgitos").max(14, "Documento deve ter no m\xE1ximo 14 d\xEDgitos").optional(),
  documentType: z32.enum(["CPF", "CNPJ"]).optional(),
  consent: z32.boolean().optional().default(true).refine((v) => v === true, { message: "Consentimento \xE9 obrigat\xF3rio" })
});
